...
<script type="text/javascript" charset="utf8" src="https://cdn.datatables.net/1.10.25/js/jquery.dataTables.js"></script>
                            ...
                            <div class="card">
                              @if(Auth::guard('admin')->id())
                                <div class="card-header border-bottom">
                                  <a href="{{route('bookrack.create')}}"> <button class="btn btn-success"> Add Books</button></a> 
                                </div>
                              @endif
                                <div class="card-datatable">
                                    <table class="dt-multilingual table">
                                        <thead>
                                            ...
                                            <tbody>
                                            @foreach($bookrack as $row)
                                            <tr>
                                            <td>ID</td>
                                             <td>{{$row->name}}</td>
                                             <td>{{Str::limit($row->descrition,10)}}</td>
  ...
  <form action="{{route('bookrack.destroy',$row->id)}}" method="post">@csrf
    @method('DELETE')
...
<a href="{{route('bookrack.edit',[$row->id])}}">
